...
inline void createOccupancyBlocksFromLayerWithInterestingness(
    const Layer<TsdfVoxel>& layer,
    const ShouldVisualizeVoxelFunctionType<TsdfVoxel>& vis_function,
    const std::string& frame_id,
    visualization_msgs::MarkerArray* marker_array) {
  CHECK_NOTNULL(marker_array);
  // Cache layer settings.
  ...
  for (const BlockIndex& index : blocks) {
    ...
    for (size_t linear_index = 0; linear_index < num_voxels_per_block;
      ...
      if (vis_function(voxel, coord)) {
        ...
        std_msgs::ColorRGBA color_msg;
        // colorVoxbloxToMsg(rainbowColorMap((coord.z() + 2.5) / 5.0), &color_msg);
        color_msg.r = voxel.interestingness;
        color_msg.g = voxel.interestingness;
        color_msg.b = 1.0;
        color_msg.a = 1.0;
        block_marker.colors.push_back(color_msg);
      }
...
inline void createOccupancyBlocksFromTsdfLayer(
    ...
    visualization_msgs::MarkerArray* marker_array) {
  CHECK_NOTNULL(marker_array);
  // std::cout << "createOccupancyBlocksFromTsdfLayer" << std::endl;
  createOccupancyBlocksFromLayerWithInterestingness(
      layer,
